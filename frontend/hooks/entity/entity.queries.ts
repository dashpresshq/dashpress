...
export function useEntityDictionPlurals<T, P extends keyof T>(input: T[], field: P) {
  const entityDictions = useApiQueries<T, { singular: string; plural: string }>({
    ...
    placeholderDataFn: (entity) => ConfigrationStorage.get('entity_diction', entity as unknown as string),
  });

  return useCallback(
    (fieldName: string): string => entityDictions.data[fieldName]
      ?.data?.plural || userFriendlyCase(fieldName),
    [entityDictions.data],
  );
}
