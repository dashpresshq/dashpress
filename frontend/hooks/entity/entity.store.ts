...
import { filterScalarEntity, getEntityReferencesMap } from 'shared/entity.logic';
...
export const useEntitiesMenuItems = () => {
  ...
  const entitiesDictionPlurals = useEntityDictionPlurals(menuItems.data || [], 'value');
...
export const useEntityScalarFields = (
  entity: string,
) => useApi<IEntityField[]>(ENTITY_FIELDS_ENDPOINT(entity), {
...
export const useEntityReferenceFields = (
  entity: string,
...
export const useEntityIdField = (entity: string) => useApi<string>(ENTITY_FIELDS_ENDPOINT(entity), {
  errorMessage: dataNotFoundMessage('Entity Scalar Fields'),
  enabled: entityEnabled(entity),
  selector: (data: IEntityField[]) => data.find(({ isId }) => isId)?.name || 'id',
