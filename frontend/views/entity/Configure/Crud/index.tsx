...
export function EntityCrudSettings() {
  ...
  const hiddenUpdateColumns = useSelectedEntityColumns('hidden_entity_update_columns');
  const hiddenDetailsColumns = useSelectedEntityColumns('hidden_entity_details_columns');
  ...
  const upsertCrudSettingsMutation = useUpsertConfigurationMutation('entity_crud_settings', entity);
  ...
  const toggleCrudSettings = (field: keyof IEntityCrudSettings) => {
    ...
    upsertCrudSettingsMutation.mutateAsync(newState as unknown as Record<string, string>);
  };
  ...
  return (
    ...
    >
      <ErrorAlert message={entityScalarFields.error || entityCrudSettings.error} />
