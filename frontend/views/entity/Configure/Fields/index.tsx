import {
  ErrorAlert, SectionBox, SortList, Spacer, Tabs, Text,
...
export function EntityFieldsSettings() {
  ...
  const { isLoading: entityFieldTypesMapIsLoading, error: entityFieldTypesMapError } = useEntityConfiguration<Record<string, string>>('entity_columns_types', entity);

  const { isLoading: entityValidationsMapIsLoading, error: entityValidationsMapError } = useEntityConfiguration<Record<string, string>>('entity_validations', entity);
          ...
          contents={[
            {
              content: (
                <>
                  <Text size="5">
                    You get the customize the labels, for the field, Say you want `updatedAt` to be
                    called `Last Updated`. Here is where you that
                  </Text>
                  <Spacer />
                  <FieldsLabelForm
            ...
            },
            {
              content: (
                ...
                <>
                  <Text size="5">
                    You get the superpowers to tell us the specific type of the fields, Say the type
                    is `email` or `url` or `textarea` as oppose to just `text` Here is where you get
                    to do that
                  </Text>
                  <Spacer />
                  ...
                  <FieldsTypeForm
                    ...
                    onSubmit={async (data) => {
                      await Promise.all([
                        ...
                        upsertEntityValidationsMutation.mutateAsync(data.validations),
                        upsertEntitySelectionsMutation.mutateAsync(data.selections || {}),
                      ]);
                    }}
            ...
            },
            {
              content: (
                <>
                  <Text size="5">
                    For some reasons, `createdAt` is showing before `userName`. This is where you
                    correct that wrong :wink
                  </Text>
                  <Spacer size="xl" />
                  <SortList
                    data={{
