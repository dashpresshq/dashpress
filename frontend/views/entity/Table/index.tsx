import {
  ComponentIsLoading, ErrorAlert, OffCanvas, Table,
...
export function EntityTable() {
  ...
  const [closeDetailsCanvas, detailsCanvasEntity, detailsCanvasId] = useDetailsOffCanvasStore(
    (state) => [state.close, state.entity, state.id],
  );

  const columns = useTableColumns();

  const error = entityCrudSettings.error || entityScalarFields.error || hiddenTableColumns.error;

  const isLoading = entityCrudSettings.isLoading
