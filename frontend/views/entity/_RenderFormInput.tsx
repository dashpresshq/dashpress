...
export const isFieldRequired = (
  entityValidationsMap: Record<string, IFieldValidationItem[]>,
  field: string,
): boolean => !!entityValidationsMap[field]?.find((item) => item.validationType === 'required');
...
export const runValidationError = (
  ...
  const validations = Object.fromEntries(
    fields.map((field) => {
      ...
      const firstFailedValidation = validationsToRun
        .find((validation) => ENTITY_VALIDATION_CONFIG[validation.validationType]?.implementation(
          values[field],
