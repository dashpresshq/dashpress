...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (req.method === 'GET') {
      return res.status(200).json(await configurationController.showConfig(key));
    }
    if (req.method === 'PUT') {
      return res.status(204).json(await configurationController.upsertConfig(key, req.body.data));
    }
  } catch (error) {
